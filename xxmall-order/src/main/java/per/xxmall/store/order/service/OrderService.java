package per.xxmall.store.order.service;
...
import per.xxmall.store.order.bean.XxmallResult;
import per.xxmall.store.order.dao.IOrder;
import per.xxmall.store.order.pojo.Order;
import per.xxmall.store.order.pojo.PageResult;
import per.xxmall.store.order.pojo.ResultMsg;
import per.xxmall.store.order.util.ValidateUtil;
...
//    private RabbitTemplate rabbitTemplate;
    ...
    public XxmallResult createOrder(String json) {
        Order order = null;
        try {
            ...
            return XxmallResult.build(400, "请求参数有误!");
        }
...
//            this.rabbitTemplate.convertAndSend(objectMapper.writeValueAsString(msg));
            ...
            return XxmallResult.ok(orderId);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return XxmallResult.build(400, "保存订单失败!");
    }
